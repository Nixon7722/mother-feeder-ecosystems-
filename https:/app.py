...
st.set_page_config(page_title="MOTHER V7", layout="wide")
st.title("🧠 MOTHER V7 - FIXED CLOSE")
...
with tab3:
    if st.button("Load Open Positions"):
        ...
        if contracts:
            ...
            for c in contracts:
                st.write(f"ID {c.get('contract_id')} | {c.get('underlying_symbol')} | Buy ${c.get('buy_price')} | P/L {c.get('profit_loss')} | {c.get('contract_type')}")
            if st.button("🔴 CLOSE ALL OPEN NOW"):
                for c in contracts:
                    otp_close = get_otp(ACCOUNT_ID)
                    r = close_contract(otp_close, c["contract_id"])
                    st.write(f"Closed {c['contract_id']}: {r}")
                st.success("All closed! Check Profit Table")
        else:
            st.json(resp)
